  ...
  return (req, res, next) => {
    try {
      ...
      if (!allowedRoles.includes(req.user.role)) {
        logger.warn(
          `Authorization failed: user ${req.user.email} with role ${req.user.role} attempted to access resource requiring ${allowedRoles.join(' or ')}`
        );
      ...
      logger.info(
        `Authorization successful: user ${req.user.email} has required role ${req.user.role}`
      );
      next();
    } catch (error) {
